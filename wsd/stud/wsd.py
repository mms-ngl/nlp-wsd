...
hononyms_path = "../../data/map/coarse_fine_defs_map.json"
...
# Label vocab
def get_hononyms(path: str) -> List[Dict]:
    labels = []

    labels.append(no_hononym)

    with open(path) as f:
        data = json.load(f)

    for hononym, hononym_data in data.items():
        labels.append(hononym)
...
def collate_fn(batch) -> Dict[str, torch.Tensor]:
    ...
    for i, label in enumerate(senses):
        ...
        for word_idx in word_ids:
            ...
            # elif word_idx != previous_word_idx and word_idx in senses_ids:
            elif word_idx != previous_word_idx:

                if word_idx in senses_ids:
                    hononym = label[str(word_idx)][0]
                    label_ids.append(label2id[hononym])
                else:
                    label_ids.append(label2id[no_hononym])
...
class Trainer():
    ...
    def train_model(self, train_dataset:Dataset, val_dataset:Dataset, epochs:int=1):
        ...
        for epoch in range(epochs):
            ...
            # torch.save(self.model.state_dict(), save_path + str(epoch+1) + '_model_weights.pth')
...
if __name__ == "__main__":
    ...
    labels = get_hononyms(hononyms_path)
